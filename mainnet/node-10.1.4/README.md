# Apex Fusion Vector mainnet relay and tooling
...
This is a relay node connected to a running `vector-mainnet` network. All `cardano-cli` commands apply as usual. For example:

To check the tip (at the moment it is about 10 min to sync, will definitely vary over time):

```
docker exec -it vector-mainnet-tools-10_1_4-vector-relay-1 cardano-cli query tip --mainnet --socket-path /ipc/node.socket
```
...
docker compose down
docker volume rm \
  vector-mainnet-tools-10_1_4_db-sync-data \
  vector-mainnet-tools-10_1_4_node-db \
  vector-mainnet-tools-10_1_4_node-ipc \
  vector-mainnet-tools-10_1_4_postgres \
  vector-mainnet-tools-10_1_4_wallet-api-data
```